import 'package:flutter/material.dart';

class AppColors {
  static Color primaryColor = Color(0xffD4E0CB);
  static Color bgColor = Color(0xfff5f5f5);
  static Color kIconColor = Colors.teal.shade800;

  static Color kPrimary = Colors.teal.shade800.withValues(alpha: 0.7);
  static Color kPrimaryLight = Colors.teal.shade50;
  static Color kPrimarySoft = Colors.teal.shade100;
  static const Color kSurface = Colors.white;
  static Color kTextMuted = Colors.teal.shade400;


  static Color kPrimaryMedium = Colors.teal.shade600;
  static const Color kSurfaceDark = Color(0xFF0F171A);
  static Color kBorderLight = Colors.grey.shade400;
  static const Color kAccentGold = Color(0xFFD4AF37);
}