...
class _RootState extends State<Root> {
  ...
  Widget _buildBottomNavigation(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final bgColor = isDark ? AppColors.kSurfaceDark : Colors.white;
    final borderColor = isDark ? Colors.white.withValues(alpha: 0.07) : AppColors.kBorderLight;
    final inactiveColor = isDark ? Colors.white.withValues(alpha: .48) : const Color(0xFF87948F);
    final activeColor = isDark ? const Color(0xFF79D6C3) : AppColors.kPrimary;
    ...
    return SafeArea(
      ...
      child: Container(
        ...
        child: Row(children: [
          _buildNavItem(index: 0, icon: Icons.home_rounded, label: 'الرئيسية', activeColor: activeColor, inactiveColor: inactiveColor),
          _buildNavItem(index: 1, icon: FlutterIslamicIcons.quran2, label: 'القرآن', activeColor: activeColor, inactiveColor: inactiveColor),
          _buildNavItem(index: 2, icon: FlutterIslamicIcons.mosque, label: 'الصلاة', activeColor: activeColor, inactiveColor: inactiveColor),
          _buildNavItem(index: 3, icon: Icons.settings_rounded, label: 'الإعدادات', activeColor: activeColor, inactiveColor: inactiveColor),
        ]),
      ),
    );
  }
        ...
        builder: (context, currentIndex, child) {
          ...
          return Semantics(
            ...
            child: GestureDetector(
              behavior: HitTestBehavior.opaque,
              onTap: () { if (!isActive) _currentIndexNotifier.value = index; },
              child: Center(
                child: AnimatedContainer(
                  duration: const Duration(milliseconds: 220),
                  curve: Curves.easeOutCubic,
                  width: isActive ? 45.w : 42.w,
                  height: 40.h,
                  padding: EdgeInsets.symmetric(horizontal: 5.w, vertical: 4.h),
                  decoration: BoxDecoration(
                    color: isActive ? activeColor.withValues(alpha: .10) : Colors.transparent,
                    shape: BoxShape.circle,
                    border: isActive ? Border.all(color: activeColor.withValues(alpha: .08)) : null,
                  ),
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    ...
                    children: [
                      AnimatedScale(
                        ...
                        child: Icon(icon, size: isActive ? 19.sp : 18.sp, color: isActive ? activeColor : inactiveColor),
                      ),
                      Gap(2.h),
                      Flexible(
                        child: Text(
