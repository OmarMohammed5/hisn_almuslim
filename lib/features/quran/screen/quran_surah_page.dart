...
class _QuranSurahPageState
  ...
  final Map<int, GlobalKey<MushafPageBlockState>>
  _pageBlockKeys = {};
  ...
  static const Color _lightAppBar =
  Color(0xFFEEE7D5);

  static const Color _darkAppBar =
  Color(0xFF1D2522);
  ...
  void _saveCurrentProgress() {
    ...
    final pages =
        quranState.pageGroup.pages;

    if (visibleIndex < 0 ||
        visibleIndex >= pages.length) {
      return;
    }
    ...
    final ayahNumber =
        blockState?.topVisibleAyahNumber(
          thresholdY: _readLineY,
        ) ??
            currentPage
    ...
    context
        ...
        .updateProgress(
      surahNumber:
      quranState.surah.number,
      page:
      currentPage.pageNumber,
      ayahNumber:
      ayahNumber,
      totalAyahsInSurah:
      quranState.surah.totalAyahs,
    );
  }
        ...
        builder: (context, state) {
          ...
          if (state is SurahPagesLoaded) {
            ...
            if (!_didAutoScroll &&
              ...
              final highlightsForSurah =
              highlightState.forSurah(
                state.surah.number,
              );

              int? targetAyah;

              if (highlightsForSurah
                  .isNotEmpty) {

                final latestHighlight =
                highlightsForSurah.entries
                    .reduce(
                      (a, b) =>
                  a.value.timestamp >
                      b.value.timestamp
